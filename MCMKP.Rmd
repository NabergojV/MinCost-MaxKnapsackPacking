...
DinamicniAlgo <- function(I,C,tabe){
  ...
  for(i in le){
    if(i <= i_c){
      ...
      tmp <- min(M[(zgornja + 1):(1 + spodnja)] + p_i[(i)])   #dodam + 1, ker se M začne pri 1 namesto pri 0
      if(tmp < OPT){
        OPT <- tmp
        i_st <- i
...
Zanima naju, kateri od algoritmov je hitrejši, zato tukaj nastavim parameter `tabe` v funkciji `DinamicniAlgo(I,C,tabe)` na `FALSE`, tako da tokrat ne oblikujem tabele, da na ta način prihranim nekaj prostora. Merjen bo tudi čas, potreben za izračun optimalne vrednosti pri maksimalni zapolnitvi.
```{r}
...
for(x in Podatki_1[c(-16, -3, -11, -2, -1)]){
  cat(x, "\n")
...
primerjava <- left_join(listi, sage, by = "name") %>% mutate(res = Time.x - Time.y) %>% select("DP" = Time.x, "MIP" = Time.y, "Razlika" = res)
kable(x = primerjava, caption = "Primerjava", format = "html") %>% 
  kable_styling("striped") %>%
  add_header_above(c("Čas merjen v sekundah" = 3))
```

Opazila sva, da se je v najnem primeru zgodilo, da je bilo računanje z `MIP` metodo hitrejše kakor z metodo `DP`, kar nasprotuje trditvam članka. Poglaviten razlog za to, bi lahko bil, da se računanje v programu sage izvaja na oblaku z vejo kapaciteto in boljčimi procesorji. V ta namen sva naredila preprost test. V vsakem programu sva zagnala zanko for na seznamu dolžine 10 milijonov ter merila čas:
```{r eval=FALSE}
k <- 0
