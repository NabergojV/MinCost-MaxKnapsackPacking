...
DinamicniAlgo <- function(I,C){
  ...
  for(i in c(length(I$w):1)){
    if(i <= i_c){
      ...
      p_i <- p_i
      zgornja <- max(0, (C - w_i))
      spodnja <- max(0, (C - w_i- I$w[i] + 1))
      tmp <- min(M[(zgornja + 1):(1 + spodnja)] + p_i)   #dodam + 1, ker se M začne pri 1 namesto pri 0
      if(tmp < OPT){
        OPT <- tmp
        i_st <- i
    ...
    #tabela <- bind_rows(tabela, data.frame(i = i, p_i = I$p[i], w_i = I$w[i], t(M), C_l = spodnja, C_u = zgornja, P_i = p_i, W_i = w_i, OPT = OPT))
  }
...
Potem ko pridobimo optimalno vrednost rešitve $OPT$, lahko zračunamo tudi optimalno rešitev $S^{\ash}$, tako da uporabimo rahlo modificiran proces vzvratnega reševanja normalnega KP (*backtracking procedure*). Spomnimo se, da nam algoritem `DinamicniAlgo(I,C)` vrne kapaciteto $C^{\ash}$ zahtevano s strani optimalne rešitve. Najprej določimo $S^{\ash} = \{ 1, \dots , i^{\ash}-1\}$, nato, začnemo pri $A_{i^{\ash}}[C^{\ash}-W_{i^{\ash}}]$, nato nadaljujemo s procesom vzvratnega reševanja za KP. 
...
def MCMKP_MIP(C,P,W):
    ...
    t = time()
    trajanje = t-s    #izmerimo čas, trajanje je v sekundah
