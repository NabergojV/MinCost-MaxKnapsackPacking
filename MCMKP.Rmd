...
Kot vhodne podatke bo `DinamicniAlgo` sprejel `I <- data.frame(w, p)` teže in cene predmetov urejenih po naraščajoči teži, torej $w_1 \leq w_2 \leq w_3 \leq \dots \leq w_n$. 
```{r DP}
DinamicniAlgo <- function(I,C){
  ...
  for(i in c(length(I$w):1)){
    if(i <= i_c){
      C_u <- max(1, (C+1 - sum(I$w[1:(i-1)])))
      C_l <- max(1, (C+1 - sum(I$w[1:i]) + 1))
      tmp <- min(M[C_u:C_l] + sum(I$p[1:(i-1)]))
      if(tmp < OPT){
        OPT <- tmp
        i_st <- i
        C_st <- (which(M == min(M[C_u:C_l] + sum(I$p[1:(i_st-1)]))) + sum(I$w[1:(i_st-1)]))
      }
    }
    for(k in c(C:I$w[i])){
