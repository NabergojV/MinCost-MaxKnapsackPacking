...
```{r}
primerjava <- left_join(listi, sage, by = "name") %>% mutate(res = OPT.x - OPT.y) %>% select("DP" = OPT.x, "MIP" = OPT.y, "Razlika OPT" = res, "C DP" = C.x, "C MIP" = C.y)
kable(x = primerjava, caption = "Primerjava OPT in C", format = "html") %>% 
  kable_styling("striped") %>%
