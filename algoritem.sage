...
C=sum(W)
def MCMKP_MIP(C,P,W):
    program = MixedIntegerLinearProgram(maximization=True,solver="GLPK")
    ...
    return program.get_values(vzamemo)

# %time nam meri čas, ki ga algoritem porabi za delovanje
%time MCMKP_MIP(C,P,W)
